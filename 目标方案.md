...
### 1.1 我们能严格保证
- **不重复分配 nonce**：同一 submitter 的 nonce 分配在任意时刻只有一个 leader 能写入（lease + fencing）。
- **不回退**：每个 submitter 的本地游标单调递增（fenced 更新），且所有状态回写幂等/有序。
...
### 2.1 同 submitter 串行化是第一性原理
- FFTM Postgres 的 `transaction_writer` 用“按 submitter 路由到同一 worker + 批处理”实现串行化与高吞吐。
- 我们在多节点场景把串行化边界升级为：集群内同 submitter 只有一个 leader（lease + fencing），节点内再用 worker 串行化减少 DB 冲突。
...
- **节点内串行化**
  - WorkerPool：`hash(submitter)%N` 路由到单线程 worker，减少同 submitter 内部并发与 DB 冲突

---

## 4. 数据模型（权威落在 Postgres）

### 4.1 submitter_lease（强制）
- 字段：`submitter, owner_node, fencing_token, expires_at`
- 语义：
...
### 4.3 submitter_nonce_cursor（建议）
- 字段：`submitter, next_nonce, fencing_token`
...
- **fenced 写入规则**：所有关键更新都必须校验：
  ...
  - owner_node == self
  - fencing_token == token
- 若 fenced（影响行数=0）：立即停止该 submitter 的处理，等待下一轮抢租

### 5.2 nonce 分配（稳定优先的规则）
- 默认：以 `submitter_nonce_cursor.next_nonce` 为主（单调递增）
- 可选：若 submitter 可能被外部系统使用，则定期参考链上 `pending nonce`（`max(dbNext, chainNext)`），以减少 nonce too low 冲突
...
### 6.1 正常路径
1. CreateTx 到达任意节点 → worker 路由到 submitter worker
...
- **后果**：两个节点同时处理同 submitter，导致重复分配/重复 resubmit/乱回写
...
### 7.7 风险：同 submitter 并发窗口过大导致后续交易全部堆在 pending
- **后果**：前序 nonce 卡住（无 receipt）时，后续 nonce 交易都会被链规则阻塞，系统表面吞吐高但实际全部 pending
- **缓解（稳定优先默认）**
  - 强制 `maxInFlight=1`（同 submitter 同时只允许 1 笔 TRACKING/ALLOCATED）
...
3. **是否允许并发窗口 >1**：稳定优先建议同 submitter `maxInFlight=1`
4. **链上 nextNonce 查询策略**：是否存在外部系统也用同 submitter（决定是否要 max(chain, db)）
...
1. **永久 STUCK 会阻塞该 submitter 的所有后续交易**：如果某 nonce 永久无法被链消耗，而你们又不允许替换/取消，只能长期卡住并人工处理。
2. **灰区 txHash 丢失无法完全自动修复**：没有“重算 txHash/txpool 查询”的 connector 能力时，只能告警人工介入。
3. **多源 submitter 使用会显著降低稳定性**：如果同 submitter 被外部系统使用，即使 max(chain, db) 也只能降低冲突概率，不能给出强一致保证；建议强约束“单一管理源”。
