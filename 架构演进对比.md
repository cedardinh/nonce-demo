...
sequenceDiagram
    ...
    Client->>Service: allocate(signer)
    Service->>Redis: tryLock(signer)
...
sequenceDiagram
    ...
    Service->>Cache: put(signer, nextNonce=11)
    Service-->>Client: allocation
```
...
```
signer_nonce_state (signer 状态)
├── signer (PK)
├── next_local_nonce
└── updated_at

signer_nonce_allocation (nonce 分配记录)
├── id (PK)
├── signer
├── nonce (UNIQUE with signer)
...
INSERT INTO managed_transaction (id, signer, nonce, tx_hash, status, fencing_token, ...)
SELECT 
    gen_random_uuid(), 
    signer, 
    nonce, 
