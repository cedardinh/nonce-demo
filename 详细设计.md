...
本详细设计用于在多实例部署下实现 **“同一 submitter 在任意时刻只有一个节点拥有执行权”**，从而保证：
...
该设计参考 `草稿.md` 中“路由分片 + 分布式租约 + fencing”思想，并吸收 FireFly Transaction Manager（FFTM）中“按 submitter 串行化 + 状态机 + receipt/confirmations 异步推进”的方法论。
...
- **执行权层（正确性）**：由“租约 lease + fencing token”决定谁能对该 submitter 做关键写入
...
- **Nonce & Tx Manager 节点（多实例）**：
  ...
  - `TransactionWriter`：按 submitter 串行/批处理写库（强建议）
...
> 说明：同一节点内部仍可能有多线程并发进入；本设计建议用“固定 worker 分片 + 单线程消费”把同一 submitter 的关键操作串行化。

```mermaid
flowchart TD
  In["HTTP/Message Input"] --> WQ["Worker Queue (hash(submitter)%N)"]
...
### 4.1 submitter 租约表（强制）

表：`submitter_lease`

| 字段 | 类型 | 说明 |
|---|---|---|
| submitter | text PK | submitter 唯一标识（地址/账户） |
...
关键索引：
- `UNIQUE (submitter, request_id)`（若 request_id 不为空）
- `INDEX (submitter, state, next_resubmit_at)`
- `INDEX (tx_hash)`

### 4.3 submitter nonce 游标（建议）

表：`submitter_nonce_cursor`

| 字段 | 类型 | 说明 |
|---|---|---|
| submitter | text PK | submitter |
...
目标：对同一 submitter，在 DB 上形成“单主”。

伪代码（概念）：

1) `BEGIN`
2) `SELECT * FROM submitter_lease WHERE submitter = ? FOR UPDATE`
3) 分支：
...
```mermaid
flowchart TD
  S["Start acquireOrRenew (submitter)"] --> L["SELECT submitter_lease FOR UPDATE"]
  L --> E{"row exists?"}
...
- 写入 SQL 形如：
  - `... WHERE submitter = :submitter AND :token = (SELECT fencing_token FROM submitter_lease WHERE submitter=:submitter AND owner_node=:self AND expires_at > now())`
  - 或者在同一事务内先锁 lease 行并校验 `owner_node/expires_at/token` 再执行写入

任何写入若校验失败（影响行数为 0）：
- 视为 **FENCED**（被栅栏拒绝）
- 立即停止该 submitter 的处理、释放本地队列、等待下一次抢租/下一次请求触发
...
同一节点内仍可能并发进入同 submitter（例如多个 HTTP 线程），需要本地串行：

- 方案 A：`ConcurrentHashMap<submitter, SingleThreadExecutor>`（简单但易膨胀）
- 方案 B：固定数量 worker（例如 256 个），`workerIndex=hash(submitter)%N`，每个 worker 单线程消费队列（推荐）

> 这与 FFTM 的 “按 submitter 分配到固定 writer worker” 同构：减少 DB 竞争、便于批处理。
...
sequenceDiagram
  ...
  Client->>Gateway: POST /tx (submitter, requestId, payload)
  Gateway->>Node: forward (load-balanced)
  Node->>Lease: acquireOrRenew(submitter)
  Lease->>DB: SELECT submitter_lease FOR UPDATE + upsert/renew
  ...
  alt not leader
    ...
    Node->>DB: SELECT managed_tx WHERE (submitter, request_id)=...
    alt duplicate
      DB-->>Node: existing tx
...
- **必须先抢租**：任何进入 nonce 分配路径前都先 `acquireOrRenewLease(submitter)`
- **幂等检查两次**：
  - 第一次：API 入口快速查 `UNIQUE (submitter, request_id)`
  - 第二次：真正写入前在同一事务内再校验（防并发窗口）
- **分配 nonce 与插入交易必须在一个事务内完成**（否则会出现“分配了但没写入”）

### 7.3 Nonce 分配策略（max(chain, db/cache)）

> 若 submitter 私钥可能被外部系统使用，必须定期参考链上 nextNonce；否则可主要依赖本地游标提升性能。

计算规则：

1) `dbNext = submitter_nonce_cursor.next_nonce`（若无则初始化）
2) `chainNext = eth_getTransactionCount(submitter, "pending")`（可配置启用）
3) `next = max(dbNext, chainNext)`
4) 分配本批交易的 nonce：从 `next` 连续递增
5) 更新 `submitter_nonce_cursor.next_nonce = next + batchCount`
...
```mermaid
flowchart TD
  A["Start allocateNonce (submitter)"] --> B["Acquire/Renew lease"]
  B --> C{"is leader?"}
  C -->|no| X["Stop: not leader"]
  C -->|yes| D["Load dbNext from submitter_nonce_cursor"]
...
### 8.2 网络分区/脑裂（两边都以为自己负责 submitter）
...
- **日志**：每条关键写入输出 `submitter/txId/nodeId/token`
- **指标**：
  - `lease_acquire_success_total{submitter?}`（submitter 建议做采样/脱敏）
  - `lease_fenced_total`
...
- 单 submitter 并发 1000 创建交易：nonce 严格递增无重复
- 多 submitter 并发：吞吐随节点数线性提升（瓶颈主要来自 DB/RPC/worker 并发度）

### 13.2 幂等

- 相同 `(submitter, requestId)` 并发 100 次：只生成 1 条交易记录
