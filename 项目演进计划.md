...
private NonceAllocation doAllocate(String submitter, String lockOwner) {
    ...
    return nonceRepository.reserveNonceWithFencing(
        submitter, targetNonce, lockOwner, 
...
INSERT INTO submitter_nonce_allocation 
    (submitter, nonce, status, lock_owner, locked_until, fencing_token, created_at, updated_at)
VALUES 
    (:submitter, :nonce, 'RESERVED', :lockOwner, :lockedUntil, :fencingToken, NOW(), NOW())
...
public ManagedTransaction createTransaction(String submitter, String requestId) {
    ...
    // 3. 分配 nonce
    NonceAllocation allocation = nonceService.doAllocate(submitter, lockOwner);
    ...
    private void reconcileSingleTransaction(ManagedTransaction tx) {
        ...
        if (chainCurrentNonce > tx.getNonce()) {
            ...
            if (txHash.isPresent()) {
                ...
                // 通知 nonce 服务：该 nonce 已被外部使用
                // （这里可能需要更新 last_chain_nonce）
            }
...
public NonceStatusResponse getStatus(@PathVariable String submitter) {
    return NonceStatusResponse.builder()
        .submitter(submitter)
        .lastChainNonce(stateRepository.getLastChainNonce(submitter))
...
public void reconcileSubmitter(String submitter) {
    ...
    // 2. 查询本地 last_chain_nonce
    SubmitterNonceState state = stateRepository.findBySubmitter(submitter);
    long localNonce = state.getLastChainNonce();
    
    // 3. 对比差异
    if (chainNonce > localNonce + 1) {
        log.warn("检测到 nonce 漂移: submitter={}, local={}, chain={}", 
            submitter, localNonce, chainNonce);
