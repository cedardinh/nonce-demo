...
在多实例部署 (GCP Load Balancer 负责流量分发) 的前提下，为同一 submitter 提供 **全局串行化的执行权**，确保：
...
> 重要澄清（与 @firefly 的工程现实一致）：
>
> - **不重复使用**、**不回退**：在“权威数据库 + lease/fencing”成立时可以做到严格保证。
> - **链上不跳号（无 gap）**：如果你禁止系统自动取消/替换/填洞交易，那么无法给出“链上永不出现 gap”的硬保证。
>   - 你能保证的是：**每个已分配 nonce 都可查可监控，且系统不会悄悄放弃它**（会持续追踪与重播同一笔业务交易，必要时升级人工介入）。
>   - 一旦某个 nonce 的交易永久无法被链消耗，后续更大 nonce 交易会被链规则阻塞，这是链的约束，不是实现缺陷。
...
语义：系统为每个 submitter 维护 nextNonce 游标；写入必须 fenced。
...
> 若 submitter 可能被外部系统使用，必须定期参考链上 `pending nonce`；否则可主要依赖 DB 游标。

```mermaid
flowchart TD
  A["开始分配 nonce (submitter)"] --> B["Acquire/Renew lease"]
  B --> C{"是 leader?"}
  C -->|no| X["停止: 非 leader"]
  C -->|yes| D["从 submitter_nonce_cursor 读取 dbNext"]
  D --> E{"启用链上查询?"}
...
### 7.4 FireFly 的处理结论与本方案差异（必须理解后再实现）

FFTM（@firefly）在 nonce 这块的实际保证是：

- **不重复使用 nonce**：通过串行化 + max(chain, db/cache) 取最大值防重用
- **允许出现 gap**：崩溃/节点丢 txpool 时可能导致本地认为已分配但链上未消耗；FFTM 用 resubmit 尽量推进，但 simple handler 不强制“取消交易填洞”

因此，本方案相较 FFTM 的关键改动是：

- **分布式执行权（lease + fencing）**：适配多节点与故障转移，硬切断旧节点写入能力
- **不引入取消/替换/填洞**：保持“业务交易语义不被系统改写”，代价是无法承诺链上无 gap，只能承诺“持续追踪 + 强告警 + 人工介入”
...
- `GET /api/v1/tx/by-request?submitter=...&requestId=...`：幂等查询

### 8.2 领域事件与回调（与业务解耦的推荐方式）

Tx Manager 对外只暴露稳定的“事件/回调协议”，业务侧无需理解 nonce/receipt/reorg 细节：

- `TxAccepted(txId, submitter, nonce)`
