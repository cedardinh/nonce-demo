...
但**前提是你能把“路由到同一台机器”升级为“同一时刻只有一个节点拥有该 submitter 的执行权”**。仅靠网关路由或一致性 hash 还不够，故障/网络分区/扩缩容时会出现短暂双活，仍会竞态。

### 你的观点哪里成立

- **稳态**（节点健康、路由稳定）：让每个节点只处理自己负责的 submitter，确实能把并发问题降到最低，基本等价于 FireFly 的“按 submitter 串行化”。

### 最大风险点

- **故障转移窗口**：旧节点“还在跑但路由已切走”、或网络分区导致两边都以为自己负责这个 submitter（脑裂）。
...
1) **路由层**：尽量把 submitter 请求打到“应负责的节点”（性能）
2) **执行权层**：最终以“租约与 fencing”决定谁能对该 submitter 做关键写入（正确性）

#### 1) 路由层怎么做

- **一致性哈希/固定分片**：`submitter -> shard -> node`
- 网关/SDK 只负责把请求尽量路由到目标节点；路由错了也没关系（下一层会挡住）。

#### 2) 执行权层怎么做（关键）

为每个 submitter 建立一个 **lease**（租约）：

- 表：`submitter_lease(submitter, owner_node, fencing_token, expires_at)`
- 规则：
  - 节点处理 submitter 前先“抢占或续租 lease”（CAS 更新 `expires_at`，并在抢占时 `fencing_token++`）。
...
- **对齐 FireFly 的核心原则**：同 submitter 的序列化是根本；你只是把序列化边界从进程内扩展到集群。
...
- 核心：把 **同一 submitter 的插入/nonce 分配请求** 路由到同一个 writer worker，从源头避免并发冲突。
- `internal/persistence/postgres/transaction_writer.go`：
  - `queue()`：插入按 `From(submitter)` 路由；更新按 `txID` 路由。
...
- `assignNonces()` 关键规则：
  ...
  - 若缓存缺失/过期：调用 `NextNonceForSubmitter` 得到链上 next nonce；再取内部 next nonce（来自过期缓存或 DB 中最高 nonce+1）；取两者最大值。
...
### 5.6 多系统共用同一 submitter：nonce 可能被外部消费导致冲突
...
- `pkg/ffcapi/next_nonce_for_submitter.go`
